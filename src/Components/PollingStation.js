...
// It handles fetching poll details, displaying candidate information, processing votes,
// showing results, and managing poll lifecycle actions (pause, resume, extend, delete) by the owner.

import React, { useState, useEffect, useCallback } from 'react';
import { useParams, useHistory, useLocation } from 'react-router-dom'; // Added useLocation
...
const PollingStation = () => {
  ...
  const { id: slugFromParams } = useParams(); // Extracts route parameter 'id' as slugFromParams.
  const history = useHistory(); // Access to browser history for navigation.
  const location = useLocation(); // Access location state for pollId
  ...
  const [isPaused, setIsPaused] = useState(false); // Tracks if the poll is currently paused by the owner.
  const [showExtendModal, setShowExtendModal] = useState(false); // Controls visibility of the extend poll duration modal.
  const [extendDays, setExtendDays] = useState(0); // Days to extend poll by.
  const [extendHours, setExtendHours] = useState(0); // Hours to extend poll by.
  const [actualPollId, setActualPollId] = useState(''); // State to store the actual poll ID
  ...
  // Effect to determine the actual poll ID from location state or decode from slug (fallback)
  useEffect(() => {
    let idToFetch = '';
    if (location.state && location.state.pollId) {
      idToFetch = location.state.pollId;
      console.log(`[PollingStation - ID Resolution] Using pollId from location state: ${idToFetch}`);
    } else if (slugFromParams) {
      // Fallback: if no state, try to decode (assuming slug might be the encoded prompt if direct navigation)
      // This might be less reliable if slugs are not simply encoded prompts.
      // Ideally, the app should always navigate with state.
      try {
        idToFetch = decodeURIComponent(slugFromParams);
        console.warn(`[PollingStation - ID Resolution] Fallback: Decoded pollId from slug: ${idToFetch}. Direct navigation or refresh might cause issues if slug is not the original prompt.`);
      } catch (e) {
        console.error("[PollingStation - ID Resolution] Error decoding slugFromParams as a fallback: ", e);
        setError('Invalid poll identifier in URL.');
        setIsLoading(false);
        return;
      }
    } else {
      console.error("[PollingStation - ID Resolution] No pollId in location state and no slug in params.");
      setError('Poll identifier is missing.');
      setIsLoading(false);
      return;
    }
    setActualPollId(idToFetch);
  }, [location.state, slugFromParams]);

  // Callback to fetch poll details from the blockchain using the actualPollId.
  // Uses useCallback to memoize the function, preventing unnecessary re-renders if actualPollId doesn't change.
  const fetchPollDetails = useCallback(async () => {
    setIsLoading(true); // Set loading state to true before fetching.
    setError(''); // Clear any previous errors.

    // Validate that actualPollId is available before attempting to fetch.
    if (!actualPollId) {
      console.error("[PollingStation - fetchPollDetails] actualPollId is not set. Cannot fetch poll.");
      // Do not set error here if it's already set by the ID resolution effect
      if (!error) setError('Poll ID is missing. Cannot load poll details.');
      setIsLoading(false);
      setPoll(null);
    ...
    try {
      ...
      console.log(`[PollingStation - fetchPollDetails] Fetching poll with actual ID: '${actualPollId}'. Slug in URL: '${slugFromParams}'.`);
      const pollData = await pollService.getPollById(actualPollId);
      if (pollData) {
        ...
        console.warn(`[PollingStation - fetchPollDetails] Poll with actual ID '${actualPollId}' not found by service.`);
      }
    } catch (err) {
      // Handle any errors during the fetch operation.
      console.error(`[PollingStation - fetchPollDetails] Error fetching poll details for actual ID '${actualPollId}':`, err);
  ...
  }, [actualPollId, slugFromParams, error]); // Dependency: actualPollId, slugFromParams, error (to avoid re-fetch if error already set)

  // Effect hook: Fetches poll details when the component mounts or when actualPollId changes.
  useEffect(() => {
    if (actualPollId) { // Only fetch if actualPollId is determined
        fetchPollDetails();
    }
  }, [actualPollId, fetchPollDetails]); // Dependency: actualPollId and memoized fetchPollDetails callback.

  // Effect hook: Checks if the current user has already voted in this poll.
  // Runs when poll data is available, user is logged in, and actualPollId is valid.
  useEffect(() => {
    if (poll && isLoggedIn && actualPollId) {
      const checkVoteStatus = async () => {
        try {
          ...
          const votedCandidateIndex = await pollService.getUserVote(actualPollId, window.accountId);
          ...
          console.error(`[PollingStation - checkVoteStatus] Error checking vote status for poll '${actualPollId}':`, error);
        }
      };
      checkVoteStatus();
    }
  }, [poll, isLoggedIn, actualPollId]); // Dependencies: poll data, login status, and actualPollId.
  ...
  useEffect(() => {
    ...
    if (poll) {
      // Set the pause status based on poll.isPaused, defaulting to false if undefined.
  ...
  const handleVote = async () => {
    ...
    if (selectedCandidateIndex === null || isVoting || hasVoted || isExpired || !actualPollId || isPaused) {
      console.warn("[PollingStation - handleVote] Vote attempt blocked due to conditions:", {
        selectedCandidateIndex, isVoting, hasVoted, isExpired, pollId: actualPollId, isPaused
      });
    ...
    const optimisticFlagKey = `lastVotedPoll_${actualPollId}_${currentAccountId}`;
    const optimisticIndexKey = `optimisticVoteIndex_${actualPollId}_${currentAccountId}`;

    try {
      console.log(`[PollingStation - handleVote] Attempting to vote for candidate ${selectedCandidateIndex} in poll '${actualPollId}' by user '${currentAccountId}'`);
      // Call pollService to submit the vote.
      await pollService.vote(actualPollId, selectedCandidateIndex);
      console.log(`[PollingStation - handleVote] Vote successful for poll '${actualPollId}'.`);
      ...
      console.error(`[PollingStation - handleVote] Error voting in poll '${actualPollId}':`, err);
  ...
  const handlePausePoll = async () => {
    if (!isOwner || !actualPollId) return; // Only owner can pause.
    
    try {
      setIsVoting(true); // Use isVoting to disable other actions during this operation.
      await pollService.pausePoll(actualPollId);
      ...
      // fetchPollDetails(); 
      console.log(`[PollingStation - handlePausePoll] Poll '${actualPollId}' paused.`);
    } catch (err) {
      console.error(`[PollingStation - handlePausePoll] Error pausing poll '${actualPollId}':`, err);
  ...
  const handleResumePoll = async () => {
    if (!isOwner || !actualPollId) return; // Only owner can resume.
    
    try {
      setIsVoting(true); // Use isVoting to disable other actions.
      await pollService.resumePoll(actualPollId);
      setIsPaused(false); // Update local pause state.
      // Optionally, refresh poll details.
      // fetchPollDetails();
      console.log(`[PollingStation - handleResumePoll] Poll '${actualPollId}' resumed.`);
    } catch (err) {
      console.error(`[PollingStation - handleResumePoll] Error resuming poll '${actualPollId}':`, err);
  ...
  const submitExtendPoll = async () => {
    if (!isOwner || !actualPollId) return; // Only owner can submit.
    ...
    try {
      ...
      await pollService.extendPoll(actualPollId, extendDays, extendHours);
      ...
      fetchPollDetails(); 
      console.log(`[PollingStation - submitExtendPoll] Poll '${actualPollId}' extended by ${extendDays}d ${extendHours}h.`);
    } catch (err) {
      console.error(`[PollingStation - submitExtendPoll] Error extending poll '${actualPollId}':`, err);
  ...
  const handleDeletePoll = async () => {
    if (!isOwner || !actualPollId) return; // Only owner can delete.
    
    // Confirm with the user before proceeding with deletion.
    if (window.confirm("Are you sure you want to delete this poll? This action cannot be undone.")) {
      try {
        setIsVoting(true); // Disable other actions.
        await pollService.deletePoll(actualPollId);
        console.log(`[PollingStation - handleDeletePoll] Poll '${actualPollId}' deleted.`);
        history.push('/'); // Redirect to home page after successful deletion.
      } catch (err) {
        console.error(`[PollingStation - handleDeletePoll] Error deleting poll '${actualPollId}':`, err);
     ...
     return (
      <div className="min-h-[calc(100vh-120px)] flex flex-col items-center justify-center bg-gradient-to-br from-slate-900 to-slate-800 text-gray-100 p-4">
        ...
        <p className="text-yellow-300 text-center max-w-md">
          ...
          {slugFromParams ? ` (Slug: ${slugFromParams}, Attempted ID: ${actualPollId || 'Error resolving ID'})` : ' (No identifier provided)'}
        </p>
        <button
          onClick={() => history.push('/')} // Navigate to home page.
          className={`${secondaryButtonClasses} mt-6`}
      ...
      <div className="max-w-3xl mx-auto bg-slate-800 shadow-2xl rounded-xl overflow-hidden">
        ...
        <div className="p-6 sm:p-8 border-b border-slate-700">
          ...
          <div className="mt-4 space-y-3 text-sm">
            ...
            {/* Row 2: Dates - Created At, Ends At - REMOVED as per request */}
            {/* 
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
              <div className="flex items-center">
                <span className="text-gray-400 mr-2 font-medium">Created:</span>
                <span className="text-gray-300">{poll.createdAt ? formatDate(parsePollDate(String(poll.createdAt))) : 'N/A'}</span>
              </div>
              {poll.durationNs && poll.createdAt && (
                  <div className="flex items-center">
                      <span className="text-gray-400 mr-2 font-medium">Ends:</span>
                      <span className="text-gray-300">
                          {formatDate(new Date(parsePollDate(String(poll.createdAt)) + Number(BigInt(String(poll.durationNs)) / 1000000n)), true)}
                      </span>
                  </div>
              )}
            </div>
        ...
        {isOwner && (
          <div className="p-4 sm:p-6 bg-slate-850 border-b border-slate-700">
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <button
                onClick={() => history.push(`/polls/edit/${slugFromParams}`, { state: { pollId: actualPollId } })} // Pass actualPollId to edit page
                  ...
                  ? poll.candidatesList.map((candidate, index) => (
                      <button
                        ...
                        className={`p-4 rounded-lg text-left transition-all duration-200 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-opacity-75
                                      ...
                                      ${hasVoted ? 'cursor-not-allowed opacity-75' : 'cursor-pointer'} flex flex-col items-start h-full`} // Added flex flex-col items-start h-full
