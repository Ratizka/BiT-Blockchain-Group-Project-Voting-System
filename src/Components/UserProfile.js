...
import { login, logout } from '../utils'; // accountId will be accessed via window.accountId
                  ...
                  <div key={poll.id} className="bg-slate-700 p-5 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 border border-slate-600 hover:border-teal-500/70">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                      <div className="flex-grow mb-3 sm:mb-0 pr-0 sm:pr-4">
                        <Link to={`/poll/${poll.id}`} className={`${linkClasses} text-lg font-semibold break-words block hover:underline`}>
                          {poll.prompt}
                        </Link>
              ...
              <ul className="space-y-4">
                {votedPolls.map((poll) => (
                  <li key={poll.id} className="p-4 bg-slate-700/50 rounded-md hover:bg-slate-600/50 transition-colors">
                    <Link to={`/poll/${poll.id}`} className={`${linkClasses} text-lg break-words block`}>
                      {poll.prompt}
                    </Link>
                    <div className="text-xs text-gray-400 mt-1.5 flex items-center justify-between">
                      ...
                      <Link to={`/poll/${poll.id}`} className="text-xs text-cyan-400 hover:text-cyan-300 flex items-center">
